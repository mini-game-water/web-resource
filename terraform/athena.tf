...
resource "aws_glue_catalog_table" "dm_activity" {
  database_name = aws_glue_catalog_database.gamehub.name
  name          = "dm_activity"
  ...
  parameters = {
    ...
    "projection.day.range"      = "1,31"
    "projection.day.digits"     = "2"
    "storage.location.template" = "s3://${aws_s3_bucket.logs.id}/dm_activity/year=$${year}/month=$${month}/day=$${day}/"
  }

  storage_descriptor {
    location      = "s3://${aws_s3_bucket.logs.id}/dm_activity/"
    input_format  = "org.apache.hadoop.mapred.TextInputFormat"
    output_format = "org.apache.hadoop.hive.ql.io.HiveIgnoreKeyTextOutputFormat"
